...
> ⚠️ **Pitfall (operational, often missed):** **Py4J and JPype** need a configured CPython environment present at runtime — typically a `venv` containing your pinned third-party packages (NumPy, transformers, your application's Python deps). Java teams expect "the JVM just runs the script" and discover at deploy time that they also have to ship a Python environment alongside the JAR. **GraalPy** has a different story: packages install into the GraalVM-managed Python distribution rather than a CPython venv, but you still package and version those dependencies — the pain shifts rather than disappearing. **Jython** runs Python 2 bytecode on the JVM with no CPython venv at all, but is unsuitable for new work anyway. Plan for two dependency-resolution stories, two reproducibility stories, and (for containers) a larger image. This pain is real and is the single biggest reason HTTP/REST often wins over tight coupling in production.
...
- **GIL invisibility and CPU-bound scaling.** The GIL ([P4 § GIL](04_concurrency.md#gil)) is invisible to out-of-process callers — a single ASGI worker (uvicorn / hypercorn) happily handles many concurrent I/O-bound requests via asyncio; the GIL only blocks parallel *Python bytecode* execution, not request concurrency itself. The implication: if your Python work is **CPU-bound** (heavy preprocessing, JSON-parsing huge payloads, in-process embedding/inference) you scale by adding **worker processes** (`uvicorn --workers N`, gunicorn) and load-balancing across them. Tighter coupling exposes the GIL to the host runtime: true in-process embeds (JPype, GraalPy, embedded CPython) hit it directly because the JVM/host thread becomes the Python thread; even local-socket bridges (Py4J) hit it because the Python side is still a single CPython process.
