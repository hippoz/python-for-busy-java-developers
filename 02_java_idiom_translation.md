...
Strings work because a Python `str` is itself a sequence — you can index it (`my_name[0]`), slice it (`my_name[1:4]`), iterate over it, and ask its length — all the operations `list` and `tuple` support. See [Part 1 § Str vs bytes](01_syntax_shock.md#str-vs-bytes) for the full story. Neither operator mutates its operands — `a + b` and `a * n` produce a result and leave `a` (and `b`) untouched. For immutable sequences, Python may reuse an existing object as an implementation detail (don't rely on object identity either way). For in-place growth on a `list`, use `list.append`, `list.extend`, or `+=` (which mutates the existing list). Java has no equivalent overloading: `int[] a + int[] b` is a compile error, and `String` concatenation with `+` is the one and only operator overload the JVM ships with.
