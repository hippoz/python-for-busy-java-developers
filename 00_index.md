...
- **Console I/O (`input` / `print(sep=, end=, file=)`)** — [P1 § Console I/O](01_syntax_shock.md#console-io)
...
- **Dict methods (`items` / `keys` / `values` / `get`)** — [P2 § Collections mapping](02_java_idiom_translation.md#collections-mapping)
...
- **Dynamic attribute addition (and how to forbid it)** — [P2 § Access conventions](02_java_idiom_translation.md#access-conventions)
- **Encapsulation (`@property`)** — [P2 § Encapsulation](02_java_idiom_translation.md#encapsulation)
- **`enum` (`Enum`/`IntEnum`/`StrEnum` + per-member values + Java-feature map)** — [P2 § Enum](02_java_idiom_translation.md#enum)
...
- **Functional patterns (`operator` / `reduce` / `partial` / Stream→Python map / `itertools` combinators)** — [P3 § Functional patterns](03_pythonic_idioms.md#functional-patterns)
...
- **`functools.reduce` / `partial`** — [P3 § Functional patterns](03_pythonic_idioms.md#functional-patterns)
...
- **`operator` module (`itemgetter` / `attrgetter` / `methodcaller`)** — [P3 § Functional patterns](03_pythonic_idioms.md#functional-patterns)
- **Operators (`/` / `//` / `%` / `**` / bitwise / chained comparison / no `++`)** — [P1 § Operators](01_syntax_shock.md#operators)
- **String methods catalog** — [P5 § Regex and strings](05_standard_library.md#regex-and-strings)
- **`super().__init__()` and inheritance** — [P2 § OOP basics](02_java_idiom_translation.md#oop-basics)
- **Ternary expression (`x if c else y`)** — [P1 § Control flow](01_syntax_shock.md#control-flow)
- **Triple-quoted strings + implicit string-literal concatenation (line-broken strings without newlines)** — [P1 § Str vs bytes](01_syntax_shock.md#str-vs-bytes)
...
- **Singleton (module / `__new__` / `@cache`)** — [P2 § Singleton](02_java_idiom_translation.md#singleton)
...
- **Variables and the object model (labels on heap objects, no primitives, automatic GC)** — [P1 § Variables and the object model](01_syntax_shock.md#variables-and-the-object-model)
