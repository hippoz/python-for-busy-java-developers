...
| Comments (`#`) + docstrings (`"""..."""`) — no `/* */` block syntax in Python; docstrings as Javadoc analog; triple-quoted-as-comment anti-pattern callout | `01#comments-and-docstrings` (new subsection) | User-flagged gap — `#` was used everywhere in examples but never documented; Java devs reach for `/* */` |
